...
def main():
    ...
    # Value represents:
    # 1) the compat version of the dependency
        ...
        if new_key is not None:
            if new_key in d:
                if "windows" in d[new_key][2]:
                    ...
                    #DEBUGGING
                    print("removed " + new_key + "because" + new_key + "'s platform is " + d[new_key][2])
                else:
                    print("did NOT remove " + new_key + "because" + new_key + "'s platform is " + d[new_key][2])
    ...
    r = requests.get("https://kojipkgs.fedoraproject.org/repos/rawhide/latest/x86_64/pkglist")
    packages = r.text
    ...
    for key in d:
        ...
        if (packages.find('/'+key+'/') != -1):
            ...
            if (packages[start_index:end_index] != version) or ('rust-' + packages[start_index:end_index] != version):
                print("    OUTDATED: The current key, " + key + " ~~~~~~~ because of comparison between:   " + packages[start_index:end_index] + " and " + version)
                outdated.append(key)
            else:
                print("NOT OUTDATED: The current key, " + key + " ~~~~~~~ because of comparison between:   " + packages[start_index:end_index] + " and " + version)
