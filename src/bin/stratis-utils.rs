...
fn predict_usage(encrypted: bool, devices: Vec<PathBuf>) -> Result<(), Box<dyn Error>> {
    ...
    let stratis_metadata_size_bytes = BDA::default().extended_size().sectors().bytes();
    ...
    let total_size: Bytes = sizes.iter().cloned().sum();

    let stratis_device_sizes = sizes
        .iter()
        .map(|&s| s - crypt_metadata_size)
        .collect::<Vec<_>>();

    let stratis_avail_sizes = stratis_device_sizes
        .iter()
        .map(|&s| s - stratis_metadata_size_bytes)
        .collect::<Vec<_>>();

    let avail_size: Bytes = stratis_avail_sizes.iter().cloned().sum();
