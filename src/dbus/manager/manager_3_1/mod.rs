...
use crate::{
    dbus::{consts, manager::Manager, manager::manager_3_0::{
        create_pool_method, destroy_pool_method, list_keys_method, set_key_method, unlock_pool_method,
        unset_key_method, version_prop, locked_pools_prop,
    }},
...
impl ManagerR1 {
    ...
    async fn DestroyPool(&self, pool: ObjectPath<'_>) -> ((bool, String), u16, String) {
        destroy_pool_method(&self.connection, &self.engine, &self.manager, pool).await
    }
    ...
    async fn UnlockPool(
        ...
        unlock_pool_method(&self.engine, pool_uuid, unlock_method).await
    }
}
