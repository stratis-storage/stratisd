...
pub use methods::{
    add_cache_devs_method, add_data_devs_method, bind_clevis_method, bind_keyring_method,
    create_filesystems_method, destroy_filesystems_method, init_cache_method, rebind_clevis_method,
    rebind_keyring_method, set_name_method, snapshot_filesystem_method, unbind_clevis_method,
    unbind_keyring_method,
};
pub use props::{
    allocated_prop, avail_actions_property, clevis_info_property, encrypted_prop,
    has_cache_property, key_description_property, name_prop, size_prop, used_prop,
};
...
impl PoolR0 {
    ...
    pub async fn unregister(
        ...
        connection.object_server().remove::<PoolR0, _>(path).await?;
        Ok(())
    }
}
