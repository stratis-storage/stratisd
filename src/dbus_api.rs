...
pub fn get_base_tree<'a>(c: &'a Connection) -> StratisResult<Tree<MethodFn<'a>>> {
    ...
    let listpools_method = f.method(LIST_POOLS, move |m, _, _| {
        ...
        .in_arg(("name", "s"));
    let createpool_method = f.method(CREATE_POOL, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let destroypool_method = f.method(DESTROY_POOL, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getpoolobjectpath_method = f.method(GET_POOL_OBJECT_PATH, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getvolumeobjectpath_method = f.method(GET_VOLUME_OBJECT_PATH, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getdevobjectpath_method = f.method(GET_DEV_OBJECT_PATH, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getcacheobjectpath_method = f.method(GET_CACHE_OBJECT_PATH, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let geterrorcodes_method = f.method(GET_ERROR_CODES, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getraidlevels_method = f.method(GET_RAID_LEVELS, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
        ...
        .in_arg(("name", "s"));
    let getdevtypes_method = f.method(GET_DEV_TYPES, move |m, _, _| {
            let mut items = m.get_items();
            if items.len() < 1 {
                return Err(MethodErr::no_arg());
            }
