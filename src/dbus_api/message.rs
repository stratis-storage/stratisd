...
use crate::{dbus_api::types::LockableTree, engine::Lockable, stratis::StratisResult};
...
pub struct DbusMessageHandler {
    connection: Lockable<Arc<RwLock<Arc<SyncConnection>>>>,
    tree: LockableTree,
}
...
impl DbusMessageHandler {
    ...
    pub async fn process_dbus_requests(&self) -> StratisResult<()> {
        ...
        let _ = self.connection.read().await.start_receive(
            ...
            Box::new(move |msg, _| {
                ...
                spawn_blocking(move || {
                    ...
                    if let Some(msgs) = lock.handle(&msg) {
                        for msg in msgs {
                            if block_on(cloned_connection.read()).send(msg).is_err() {
                        ...
                        if let Some(r) = reply {
                            if block_on(cloned_connection.read()).send(r).is_err() {
                                warn!("Failed to send reply to D-Bus client");
                            }
                        }
                    }
