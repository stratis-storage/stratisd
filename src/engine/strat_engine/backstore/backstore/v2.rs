...
use crate::{
    engine::{
        ...
        types::{
            ...
            KeyDescription, OptionalTokenSlotInput, PoolUuid, SizedKeyMemory, UnlockMechanism,
        },
        TokenUnlockMethod,
    },
...
mod tests {
    ...
    fn test_clevis_initialize(paths: &[&Path]) {
        ...
        ei.add_info(None, UnlockMechanism::ClevisInfo((
            "tang".to_string(),
    ...
    fn test_clevis_both_initialize(paths: &[&Path]) {
        fn test_both(paths: &[&Path], key_desc: &KeyDescription) {
            ...
            let pool_uuid = PoolUuid::new_v4();
            let mut ei = InputEncryptionInfo::new();
            ei.add_info(None, UnlockMechanism::KeyDesc(key_desc.to_owned()))
                .expect("Empty data structure");
            ei.add_info(None, UnlockMechanism::ClevisInfo((
                "tang".to_string(),
                json!({"url": env::var("TANG_URL").expect("TANG_URL env var required"), "stratis:tang:trust_url": true}),
