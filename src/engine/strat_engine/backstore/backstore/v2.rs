...
impl Backstore {
    ...
    pub fn volume_key_is_loaded(&self, uuid: PoolUuid) -> StratisResult<bool> {
        Ok(search_key_persistent(Either::Right(&VolumeKeyKeyDescription::new(uuid)))?.is_some())
    }

    /// Load volume key into the kernel keyring for a crypt device in the backstore.
    pub fn load_volume_key(&self, uuid: PoolUuid) -> StratisResult<bool> {
        let crypt_physical_path = &once(DEVICEMAPPER_PATH)
            .chain(once(
