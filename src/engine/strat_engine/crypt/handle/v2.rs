...
use crate::{
    engine::{
        ...
        types::{
            ...
            SizedKeyMemory, UnlockMechanism,
        },
        TokenUnlockMethod,
    },
...
impl CryptHandle {
    ...
    fn initialize_with_err(
        ...
        let (to_delete, clevis_bind_input) = match (
            key_descs.iter().next(),
            key_descs_with_token_id.keys().next(),
        ) {
            (_, Some(t)) => (None, Either::Left(*t)),
            (Some(kd), _) => {
                let key = read_key(kd)?.ok_or_else(|| {
                    StratisError::Msg(format!(
                        "Passphrase for key description {} not found in keyring",
                        kd.as_application_str()
                    ))
                })?;
                (None, Either::Right(key))
            }
            (None, None) => {
    ...
    pub fn rollback(
        ...
        if let Err(e) = ensure_wiped(device, physical_path, name) {
            ...
            Err(wipe_fallback(physical_path, e))
        } else {
            Ok(())
        }
    }
...
mod tests {
    ...
    use crate::engine::{
        strat_engine::{
            crypt::{
                consts::{
                    DEFAULT_CRYPT_KEYSLOTS_SIZE, DEFAULT_CRYPT_METADATA_SIZE, STRATIS_MEK_SIZE,
                },
    ...
    fn test_failed_init(paths: &[&Path]) {
        ...
        let mut ei = InputEncryptionInfo::new();
        ei.add_info(None, UnlockMechanism::KeyDesc(key_description))
    ...
    fn test_crypt_device_ops(paths: &[&Path]) {
        fn crypt_test(paths: &[&Path], key_desc: &KeyDescription) {
            ...
            let mut ei = InputEncryptionInfo::new();
            ei.add_info(Some(0), UnlockMechanism::KeyDesc(key_desc.clone()))
                .expect("Empty data structure");
    ...
    fn loop_test_set_sector_size() {
        fn the_test(paths: &[&Path]) {
            fn test_set_sector_size(paths: &[&Path], key_description: &KeyDescription) {
                ...
                let mut ei = InputEncryptionInfo::new();
                ei.add_info(Some(0), UnlockMechanism::KeyDesc(key_description.clone()))
                    .expect("Empty data structure");
    ...
    fn test_both_initialize(paths: &[&Path]) {
        fn both_initialize(paths: &[&Path], key_desc: &KeyDescription, pool_uuid: PoolUuid) {
            ...
            ei.add_info(Some(0), UnlockMechanism::KeyDesc(key_desc.clone()))
                .expect("Empty data structure");
            ei.add_info(Some(1), UnlockMechanism::ClevisInfo(
                (
                    "tang".to_string(),
    ...
    fn test_clevis_initialize(paths: &[&Path]) {
        ...
        let mut ei = InputEncryptionInfo::new();
        ei.add_info(Some(0), UnlockMechanism::ClevisInfo(
            (
                "tang".to_string(),
                json!({"url": env::var("TANG_URL").expect("TANG_URL env var required"), "stratis:tang:trust_url": true}),
            ),
