...
use crate::{
    engine::{
        ...
        strat_engine::{
            ...
            keys::{get_persistent_keyring, read_key_persistent},
...
fn setup_crypt_handle(
        ...
        .exists()
    {
        ...
        if let Err(e) = load_vk_to_keyring(device, passphrase, pool_uuid) {
...
impl CryptHandle {
    ...
    pub fn resize(&mut self, uuid: PoolUuid, size: Option<Sectors>) -> StratisResult<()> {
        ...
        let (_, vk) = read_key_persistent(Either::Right(&vk_kd))?.ok_or_else(|| {
            StratisError::Msg(format!(
        ...
        crypt.activate_handle().set_keyring_to_link(
