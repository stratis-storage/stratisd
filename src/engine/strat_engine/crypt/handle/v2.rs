...
impl CryptHandle {
    ...
    pub fn resize(&mut self, uuid: PoolUuid, size: Option<Sectors>) -> StratisResult<()> {
        ...
        let (_, vk) =
            read_key_persistent(Either::Right(format!("stratis-vk-pool-{}", uuid).as_str()))?
                .ok_or_else(|| {
                    StratisError::Msg(format!(
        ...
        crypt.activate_handle().set_keyring_to_link(
            format!("stratis-vk-pool-{}", uuid.to_string()).as_str(),
            None,
            None,
            Some("@s"),
        )?;
