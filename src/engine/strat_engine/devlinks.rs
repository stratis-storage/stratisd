...
use crate::{engine::engine::DEV_PATH, stratis::StratisResult};

#[cfg(not(test))]
pub use real_methods::*;
#[cfg(test)]
...
mod test_methods {
    ...
    use std::{collections::HashSet, io::ErrorKind, os::unix::fs::symlink, path::Path};
    ...
    /// Clean up directories and symlinks under /dev/stratis based on current
    ...
    // (e.g. user shells) with the current working directory within the tree.
    pub fn cleanup_devlinks<'a, I: Iterator<Item = (&'a Name, &'a PoolUuid, &'a StratPool)>>(
        pools: I,
    ) {
        if let Err(err) = || -> StratisResult<()> {
