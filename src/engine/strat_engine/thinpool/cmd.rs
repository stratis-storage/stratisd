...
pub fn create_fs(devnode: &Path, uuid: Uuid) -> StratisResult<()> {
    execute_cmd(
        Command::new([PATH_PREFIX, "mkfs.xfs"].iter().collect::<PathBuf>())
            .arg("-f")
            .arg("-q")
            .arg(&devnode)
            .arg("-m")
...
pub fn xfs_growfs(mount_point: &Path) -> StratisResult<()> {
    execute_cmd(
        Command::new([PATH_PREFIX, "xfs_growfs"].iter().collect::<PathBuf>())
            .arg(mount_point)
            .arg("-d"),
...
pub fn set_uuid(devnode: &Path, uuid: Uuid) -> StratisResult<()> {
    execute_cmd(
        Command::new([PATH_PREFIX, "xfs_admin"].iter().collect::<PathBuf>())
            .arg("-U")
            .arg(format!("{}", uuid))
            .arg(&devnode),
...
pub fn thin_check(devnode: &Path) -> StratisResult<()> {
    execute_cmd(
        Command::new([PATH_PREFIX, "thin_check"].iter().collect::<PathBuf>())
            .arg("-q")
            .arg(devnode),
...
pub fn thin_repair(meta_dev: &Path, new_meta_dev: &Path) -> StratisResult<()> {
    execute_cmd(
        Command::new([PATH_PREFIX, "thin_repair"].iter().collect::<PathBuf>())
            .arg("-i")
            .arg(meta_dev)
            .arg("-o")
            .arg(new_meta_dev),
