...
impl StratisParams {
    async fn process(self, engine: EngineType) -> StratisRet {
        match self.type_ {
...
impl StratisServer {
    pub fn new<P>(engine: EngineType, path: P) -> StratisResult<StratisServer>
    where
        P: AsRef<Path>,
    {
...
pub fn run_server(engine: EngineType) -> JoinHandle<()> {
    tokio::spawn(async move {
