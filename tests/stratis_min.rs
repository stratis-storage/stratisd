...
#[test]
// stratis-min should fail without using the IPC when setting a key if the
// user-specified keys are nothing but whitespace.
...
#[test]
// Test that when an empty key is specified on pool start, the key is
// still sent over the IPC mechanism and the pool fails to start.
